...
bool WiFi_TX::transmit_beacon(ODID_UAS_Data &UAS_data)
{
    ...
    if ((length = odid_wifi_build_message_pack_beacon_frame(&UAS_data,(char *)WiFi_mac_addr,
        ...
        uint8_t ie_storage[sizeof(vendor_ie_data_t) + 1024] {};
        auto *IE_data = reinterpret_cast<vendor_ie_data_t *>(ie_storage);
...
bool WiFi_TX::stop(void)
{
    if (!initialised || !beacon_ie_enabled) {
        return true;
    }

    vendor_ie_data_t empty_ie {};
    empty_ie.element_id = WIFI_VENDOR_IE_ELEMENT_ID;
    empty_ie.length = 4;
    empty_ie.vendor_oui[0] = 0xFA;
    empty_ie.vendor_oui[1] = 0x0B;
    empty_ie.vendor_oui[2] = 0xBC;
    empty_ie.vendor_oui_type = 0x0D;

    const bool ok_beacon = (esp_wifi_set_vendor_ie(false, WIFI_VND_IE_TYPE_BEACON, WIFI_VND_IE_ID_0, &empty_ie) == ESP_OK);
    const bool ok_probe = (esp_wifi_set_vendor_ie(false, WIFI_VND_IE_TYPE_PROBE_RESP, WIFI_VND_IE_ID_0, &empty_ie) == ESP_OK);
    beacon_ie_enabled = false;
    return ok_beacon && ok_probe;
}
